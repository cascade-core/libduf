...
/**
 * Decodes/Encodes URL-encoded values entered into field. For example when
 * elFinder selector populates field with encoded URL fragment. This processor
 * uses urlencode and urldecode funcitons.
 */
class UrlEncoder
{
	...
	public static function valuePreProcess($default_values, & $raw_values, \Duf\Form $form, $group_id, $field_id, $field_conf)
	{
		if (isset($default_values[$field_id])) {
			$raw_values[$field_id] = urlencode($default_values[$field_id]);
		}
	}
